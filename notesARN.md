...
1. Exploratory Data Analysis ; regarder les données, déterminer si les outliers sont des erreurs ou peuvent être enlevés, car outliers perturbe l'entrainement
...
Supervised learning : data collection -> pre-processing -> feature extraction -> model selection -> perf evaluation
...
Gradient descent: new model; ADALINE (adaptive linear element)
y = sum(wixi)
...
Widrow-Hoff algorithm / Delta rule : Gradient descent
...
Overfitting: si on a trop de neurones, ça peut créer une fonction qui est super bizarre et précise -> overfitting, on peut distinguer quasiment chacun des points.
...
Data normalization: toutes les données entre 0 et 1, ou -1 et 1: x' = (x-xmin)/(xmax-xmin)
...
Unrepresentative training dataset: dataset d'entrainement trop petit ou trop peu diversifié ou biaisé.

CNN typical architecture : feature maps (matrice A x matrice B), réduction à chaque feature map sur les bords. 9 pixels into 1 pixel
n feature map, peut avoir des gros mismatches.
Convolution layers : 9 x, 9 poids + 1 biais -> into 1 neurone y = sum(wixi) + b

Maxpooling: reduce  a n:n matrix to a n/m:n/m matrix by taking the max value within m^2 cases.
